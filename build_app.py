...
async function fetchReviewsViaServer(locations, googleApiKey, locationNames){
  ...
  try{
    const resp=await apiFetch('/api/fetch-reviews',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({locations,googleApiKey:googleApiKey||'',locationNames:locationNames||{}})});
    if(!resp.ok){const e=await resp.json().catch(()=>({}));return {ok:false,error:e.error||'Server returned '+resp.status}}
...
async function syncReviewsFromServer(){
  ...
  try{
    const resp=await apiFetch('/api/reviews?limit=5000');
    if(!resp.ok) return;
    const data=await resp.json();
