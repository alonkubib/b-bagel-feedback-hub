...
function IntegrationsPage({onNavigate,onPasteExtract}){
  ...
  const [scanDates,setScanDates]=useState(()=>{try{return JSON.parse(localStorage.getItem('fhub_last_scan')||'{}')}catch(e){return{}}});

  const getLastScan=(platformId)=>scanDates[platformId]||null;
  const setLastScan=(platformId,date)=>{
    const d={...scanDates,[platformId]:date};
    setScanDates(d);
    try{localStorage.setItem('fhub_last_scan',JSON.stringify(d))}catch(e){}
  };
